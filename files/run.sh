...
CASSANDRA_CONFIGMAP_DIR=/tmp/cassandra/configmap
CASSANDRA_CFG=$CASSANDRA_CONF_DIR/cassandra.yaml

if [ -d $CASSANDRA_CONFIGMAP_DIR ]; then
    echo "We have a ConfigMap, we surcharge default configuration files"
    cp -v $CASSANDRA_CONFIGMAP_DIR/* $CASSANDRA_CONF_DIR/
fi

if [ -f $CASSANDRA_CONFIGMAP_DIR/pre_run.sh ]; then
    echo "We found pre-run.sh script, we execute it"
    $CASSANDRA_CONFIGMAP_DIR/pre_run.sh
fi
...
if [ -n "$CASSANDRA_MAX_HEAP" ]; then
    sed -ri -e "s/^(#)?-Xmx[0-9]+.*/-Xmx$CASSANDRA_MAX_HEAP/"  \
        -e "s/^(#)?-Xms[0-9]+.*/-Xms$CASSANDRA_MAX_HEAP/" "$CASSANDRA_CONF_DIR/jvm.options"
fi
...
if [[ $CASSANDRA_ENABLE_JMX == 'true' ]]; then
  ...
  if [[ $CASSANDRA_ENABLE_JOLOKIA == 'true' ]]; then
      JAVA_AGENT="-javaagent:/usr/local/share/jolokia-agent.jar=host=0.0.0.0,executor=fixed"
      ...
      cat  <<EOF >>$CASSANDRA_CONF_DIR/cassandra-env.sh
